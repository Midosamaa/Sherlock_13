...
int main(int argc, char *argv[])
{
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
				...
				// Si le nombre de joueurs atteint 4, alors on peut lancer le jeu
                                ...
                                if (nbClients==4)
				{
										// Distribuer les cartes et les symboles à chaque joueur
										for (int j = 0; j < 4; j++) {
											int c1 = deck[j * 3];
											int c2 = deck[j * 3 + 1];
											int c3 = deck[j * 3 + 2];

											// Message D : cartes du joueur
											sprintf(reply, "D %d %d %d", c1, c2, c3);
											sendMessageToClient(tcpClients[j].ipAddress, tcpClients[j].port, reply);

											// Messages V : ligne de tableCartes
											for (int column = 0; column < 8; column++) {
												sprintf(reply, "V %d %d %d", j, column, tableCartes[j][column]);
												sendMessageToClient(tcpClients[j].ipAddress, tcpClients[j].port, reply);
											}
										}

										// Message M : joueur courant = 0
	...
	else if (fsmServer==1)
	{
		...
		{
			case 'G': {
				// Gestion d'une accusation
				int joueur, accuse;
				sscanf(buffer, "G %d %d", &joueur, &accuse);
			
				int vraiCoupable = deck[12];
			
				if (accuse == vraiCoupable) {
					// Le joueur a bien deviné
					sprintf(reply, "W %d", joueur);
					broadcastMessage(reply);
					exit(0);  // Fin de la partie
				} else {
					// Mauvaise accusation
					sprintf(reply, "X %d", joueur);
					broadcastMessage(reply);
			...
			case 'O': {
				// Demande d’un objet à tous sauf soi-même
				int joueur, symbole;
				sscanf(buffer, "O %d %d", &joueur, &symbole);
			
				int somme = 0;
				for (int i = 0; i < 4; i++) {
					if (i != joueur) {
						somme += tableCartes[i][symbole];
					}
				}
			
				// Répondre uniquement au joueur demandeur
				sprintf(reply, "R %d", somme);
			...
			case 'S': {
				// Question ciblée à un autre joueur sur un objet précis
				int joueur, cible, symbole;
				sscanf(buffer, "S %d %d %d", &joueur, &cible, &symbole);
			
				int quantite = tableCartes[cible][symbole];
			
				// Envoyer la réponse uniquement au joueur demandeur
				sprintf(reply, "R %d", quantite);
				sendMessageToClient(tcpClients[joueur].ipAddress, tcpClients[joueur].port, reply);
